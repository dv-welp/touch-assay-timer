...
finishTrialButton.addEventListener("click", () => {
  const confirmed = confirm(
    "Finish this trial?\n\n" +
...
exportExcelButton.addEventListener("click", () => {
  ...
  checked.forEach(input => {
    ...
    if (type === "trial") {
      ...
      // BINNED
      const binned2D = buildTrialBinned2D(trial, currentAssay);
      const binnedSheet = XLSX.utils.aoa_to_sheet(binned2D);
      applySheetLayout(binnedSheet, binned2D);
      XLSX.utils.book_append_sheet(
        wb,
        binnedSheet,
        `Trial_${trialIndex}_Binned`
      );
    }
    ...
    if (type === "pooled") {
      ...
      // BINNED
      const binned2D = buildPooledBinned2D(currentAssay, {
        includeAbandoned
      });
      const binnedSheet = XLSX.utils.aoa_to_sheet(binned2D);
      applySheetLayout(binnedSheet, binned2D);
...
function warmUpAudio() {
  ...
  if (run.ineligibleReason) {
    return `Ineligible (${run.ineligibleReason})`;
  }

  switch (run.status) {
    case "stoppedEarly":
      return "Ineligible (Stopped Early)";
    case "abandoned":
      return "Ineligible (Abandoned)";
    default:
      return "Ineligible";
  }
}
...
function exportAssayXLSX(options = {}) {
  if (!currentAssay) {
    alert("No assay to export.");
    return;
  }
  ...
  // ---------- per-trial sheets ----------
  currentAssay.trials.forEach(trial => {
    const trialIndex = trial.trialIndex;

    // RAW
    const raw2D = buildTrialRaw2D(trial, currentAssay);
    const rawSheet = sheetFrom2DArray(raw2D);
    applySheetLayout(rawSheet, raw2D);
    XLSX.utils.book_append_sheet(
      wb,
      rawSheet,
    ...
    // BINNED
    const binned2D = buildTrialBinned2D(trial, currentAssay);
    const binnedSheet = sheetFrom2DArray(binned2D);
    applySheetLayout(binnedSheet, binned2D);
    XLSX.utils.book_append_sheet(
      wb,
      binnedSheet,
  ...
  // ---------- pooled sheets ----------
  const pooledRaw2D = buildPooledRaw2D(currentAssay, options);
  const pooledRawSheet = sheetFrom2DArray(pooledRaw2D);
  applySheetLayout(pooledRawSheet, pooledRaw2D);
  XLSX.utils.book_append_sheet(
    wb,
    pooledRawSheet,
    "Pooled_Raw"
  ...
  const pooledBinned2D = buildPooledBinned2D(currentAssay, options);
  const pooledBinnedSheet = sheetFrom2DArray(pooledBinned2D);
  applySheetLayout(pooledBinnedSheet, pooledBinned2D); 
  XLSX.utils.book_append_sheet(
    wb,
    pooledBinnedSheet,
    "Pooled_Binned"
...
async function updateRun(assayId, trialId, run) {
  ...
  const db = await openDB();
  const tx = db.transaction(STORES.RUNS, "readwrite");
  tx.objectStore(STORES.RUNS).put(run);
  return tx.complete;
}
